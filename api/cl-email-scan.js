...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 'gmail:<message_id>'). Legacy rows with a NULL source_unique_key
    ...
    if (messages.length > 0) {
      var msgKeys = messages.map(function(m) { return buildSourceUniqueKey('gmail', { message_id: m.id }); });
      ...
      if (existingRes.error) {
        ...
        messages = messages.filter(function(m) {
          return !existingKeys.has(buildSourceUniqueKey('gmail', { message_id: m.id }));
        });
        console.log('[Gmail] Pre-filtered — already in cl_source_items:', beforeCount - messages.length, 'remaining:', messages.length);
      }
    }
    ...
    for (const msg of batch) {
      ...
      var msgItemCount = 0;
      var emailSourceKey = buildSourceUniqueKey('gmail', { message_id: msg.id });
      ...
      if (uploadRes && uploadRes.error) {
        console.error('[Gmail] Storage upload failed — msgId:', msg.id, 'error:', uploadRes.error.message);
        skipped++;
      ...
      if (!sourceItemId) {
        console.error('[Gmail] Source row failed — msgId:', msg.id);
        skipped++;
        skipped_reasons.source_row_failed = (skipped_reasons.source_row_failed || 0) + 1;
        continue;
      }
      ...
      for (var attIdx = 0; attIdx < attachmentParts.length; attIdx++) {
        ...
        var attSourceKey = buildSourceUniqueKey('gmail-attachment', {
          message_id: msg.id,
        ...
        if (attUploadRes && attUploadRes.error) {
          console.error('[Gmail Attachment] Storage upload failed — msgId:', msg.id, 'filename:', att.filename, 'error:', attUploadRes.error.message);
          skipped++;
          skipped_reasons.source_row_failed = (skipped_reasons.source_row_failed || 0) + 1;
          continue;
        }
        ...
        if (!attSourceItemId) {
          console.error('[Gmail Attachment] Source row failed — msgId:', msg.id, 'filename:', att.filename);
          skipped++;
          skipped_reasons.source_row_failed = (skipped_reasons.source_row_failed || 0) + 1;
          continue;
        }
