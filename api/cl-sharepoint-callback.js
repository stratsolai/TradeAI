// api/cl-sharepoint-callback.js
// OAuth callback for SharePoint — Task 10 CL Connections.
...
const REDIRECT_URI = 'https://staxai.com.au/api/cl-sharepoint-callback';
const SCOPES = 'Sites.Read.All offline_access User.Read';
...
function redirectError(res, details) {
  return res.redirect('/cl-settings.html?error=sharepoint_failed&details=' + encodeURIComponent(details || 'unknown'));
}
...
export default async function handler(req, res) {
  ...
  if (error) {
    console.error('SharePoint OAuth error from provider:', error, error_description);
    return redirectError(res, error_description || error);
  }

  if (!code) {
    console.error('SharePoint callback hit with no code');
  ...
  if (!userId) {
    console.error('SharePoint callback could not resolve userId from state');
    return redirectError(res, 'no_user');
  }

  if (!MICROSOFT_CLIENT_ID || !MICROSOFT_CLIENT_SECRET) {
    console.error('SharePoint callback: Microsoft credentials not configured');
  ...
  try {
    ...
    if (tokenData.error || !tokenData.access_token) {
      console.error('SharePoint token exchange failed:', tokenData.error, tokenData.error_description);
    ...
    try {
      ...
      console.error('SharePoint /me lookup failed:', meErr && meErr.message);
    }

    if (!accountEmail) {
      console.error('SharePoint callback: could not determine account email from /me');
      return redirectError(res, 'no_account_email');
    }

    // 3. Read existing cl_sharepoint_accounts array
    ...
    if (profileRes.error) {
      console.error('SharePoint callback: profile read failed:', profileRes.error.message);
      return redirectError(res, profileRes.error.message);
    }

    const currentAccounts = Array.isArray(profileRes.data && profileRes.data.cl_sharepoint_accounts)
    ...
    const updateRes = await supabase
      .from('profiles')
      .update({ cl_sharepoint_accounts: currentAccounts })
      .eq('id', userId);

    if (updateRes.error) {
      console.error('SharePoint profile update failed:', updateRes.error.message);
      return redirectError(res, updateRes.error.message);
    }

    return res.redirect('/cl-settings.html?connected=sharepoint');
  } catch (err) {
    console.error('SharePoint callback exception:', err && err.message ? err.message : err);
