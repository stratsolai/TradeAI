...
async function runExtractionPrompt(content, fileName, userId) {
  ...
  return sharedRunExtractionPrompt({
    ...
    userId: userId,
    subtype: 'dropbox-extraction',
    errorScope: 'CL Dropbox',
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'import-all') {
      ...
      // 'dropbox:<dropbox_file_id>'). The Dropbox file id is the stable
      // "id:Aa1bBcDe2fG" form that survives renames and moves. Legacy rows
      // with NULL source_unique_key won't match — those files will fall
      // through and dedup again at the content_library row level until the
      // Step 8 backfill runs.
      var fileKeys = allFiles
        .filter(function(f) { return !!f.id; })
        .map(function(f) { return buildSourceUniqueKey('dropbox', { dropbox_file_id: f.id }); });
      ...
      if (fileKeys.length > 0) {
        ...
        if (existingSI && existingSI.error) {
          console.error('[Dropbox] Pre-filter query error:', existingSI.error.message);
      ...
      const files = allFiles.filter(function(f) {
        if (!f.id) return true;
        return !scannedKeys.has(buildSourceUniqueKey('dropbox', { dropbox_file_id: f.id }));
      });
      ...
      for (const file of batch) {
        ...
        // crash don't get blocked by the existing object. Storage path strips
        // Dropbox file id colons since cl-assets paths do not allow them.
        if (!file.id) {
          console.error('[Dropbox] Source row failed — missing file id, path:', filePath);
          skipped++;
        ...
        var fileItemCount = 0;
        var safeFileId = file.id.replace(/[^a-zA-Z0-9]/g, '_');
        var safeFileName = fileName.replace(/[^a-zA-Z0-9._-]/g, '_');
        var storagePath = userId + '/dropbox/' + safeFileId + '_' + safeFileName;
        var fileSourceKey = buildSourceUniqueKey('dropbox', { dropbox_file_id: file.id });
        ...
        if (uploadRes && uploadRes.error) {
          console.error('[Dropbox] Storage upload failed — fileId:', file.id, 'error:', uploadRes.error.message);
          skipped++;
          skipped_reasons.source_row_failed = (skipped_reasons.source_row_failed || 0) + 1;
          continue;
        }
        ...
        if (!sourceItemId) {
          console.error('[Dropbox] Source row failed — fileId:', file.id);
          skipped++;
          skipped_reasons.source_row_failed = (skipped_reasons.source_row_failed || 0) + 1;
          continue;
        }
