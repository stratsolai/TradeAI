...
async function extractImage(fileData, apiKey, mediaType) {
  const body = JSON.stringify({
    ...
    messages: [{ role: 'user', content: [
      ...
      { type: 'text', text: 'This is an image file. First determine whether this is a photo (scene, people, objects, work) or a document/screenshot (contains text that should be read).\n\nFor photos: write a plain English visual description of what is shown — what was done, the setting, visible quality or detail. Do not invent detail that cannot be seen. Use this description as the body field.\n\nFor documents or screenshots: extract all visible text accurately and use it as the body field. For this type of image the body MUST contain the extracted text — this overrides the summary-only instruction in the system prompt because the visible text IS the source content and there is no other way to capture it.\n\nThen apply the standard extraction instructions: classify into a CL category, generate a short descriptive title (maximum 10 words), assign tool tags, and make a confidence decision. Return the result in the same JSON format as all other file types. You must always return at least one item — never return an empty array for an image that contains visible content.' }
    ]}]
  });
