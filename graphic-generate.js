...
// Builds a multipart/form-data body for the Ideogram remix endpoint
// (which requires binary image upload, not base64 JSON)

function buildMultipartForm(fields, imageBuffer, imageFilename, imageContentType) {
  const boundary = `----TradeAIBoundary${Date.now()}`;
  const parts = [];
...
// POST multipart form data to Ideogram
...
async function buildImagePrompt(claudeApiKey, { postContent, category, businessName, industry, platforms, brandColours, hasUserPhoto }) {
...
${hasUserPhoto
  ? `- IMPORTANT: A real customer photo is being used as the base image. Write a prompt that ENHANCES and STYLES the existing photo into a polished marketing graphic. Focus on: professional lighting enhancement, clean composition, bold colour grading, and making the workmanship look impressive. Do NOT describe a new scene — describe how to elevate the existing photo.`
  : `- Generate an entirely original creative scene from scratch`
}
...
${hasUserPhoto ? `Mode: PHOTO ENHANCEMENT — the customer has uploaded their own photo. Write a remix/enhancement prompt.` : `Mode: ORIGINAL GENERATION — create a scene from scratch.`}

Post content (use this for context only):
"${(postContent || '').substring(0, 400)}"

Write a creative, specific image generation prompt.`;
...
module.exports = async (req, res) => {
  ...
  const {
    ...
    brandColours,
    campaignName,
    imageBase64       // Optional — customer's own photo (base64 string, no data URI prefix needed)
  } = req.body;
  ...
  try {
    ...
    const hasUserPhoto = !!(imageBase64 && imageBase64.length > 100);
    console.log('[graphic-generate] Building image prompt... hasUserPhoto:', hasUserPhoto);
    const imagePrompt = await buildImagePrompt(claudeKey, {
      postContent, category, businessName, industry, platforms, brandColours, hasUserPhoto
    });
    console.log('[graphic-generate] Prompt:', imagePrompt);

    const aspectRatio = getAspectRatio(platforms);
    let imageUrl;

    if (hasUserPhoto) {
      // ── 3a. REMIX — customer uploaded their own photo ─────────────────
      console.log('[graphic-generate] Using remix (customer photo)...');

      // Strip data URI prefix if present, decode to buffer
      const base64Clean = imageBase64.includes(',') ? imageBase64.split(',')[1] : imageBase64;
      const imageBuffer = Buffer.from(base64Clean, 'base64');

      // Detect format from data URI or default to jpeg
      const mimeMatch = imageBase64.match(/data:(image\/\w+);/);
      const mimeType  = mimeMatch?.[1] || 'image/jpeg';
      const extension = mimeType.split('/')[1] || 'jpg';

      const { body: formBody, boundary } = buildMultipartForm(
        {
          prompt:           imagePrompt,
          aspect_ratio:     aspectRatio,
          rendering_speed:  'BALANCED',
          magic_prompt_option: 'OFF',
          image_weight:     70,   // 70 = strong influence from original photo, still enhanced
          negative_prompt:  'blurry, low quality, distorted, ugly, overexposed, watermark, text, logo'
        },
        imageBuffer,
        `upload.${extension}`,
        mimeType
      ...
      const remixResp = await ideogramMultipart(
      ...
      if (remixResp.status !== 200) {
        ...
        return res.status(500).json({ error: 'Ideogram remix API error', details: remixResp.body });
      }

      imageUrl = remixResp.body?.data?.[0]?.url;

    } else {
      // ── 3b. GENERATE — no photo, create from scratch ──────────────────
      console.log('[graphic-generate] Using generate (no photo)...');

      const generateResp = await httpsPost(
        'api.ideogram.ai',
      ...
      if (generateResp.status !== 200) {
        ...
        return res.status(500).json({ error: 'Ideogram API error', details: generateResp.body });
      }

      imageUrl = generateResp.body?.data?.[0]?.url;
    }
