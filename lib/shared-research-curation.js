...
function buildDiagnosticUserMessage(cohortContext, droppedItems) {
  ...
  const compact = droppedItems.map((it, i) => ({
    i,
    t: it.title || '',
    s: (it.body || it.snippet || '').slice(0, DIAGNOSTIC_INPUT_TRUNCATE),
    u: it.link || it.url || '',
    src: it.source || it.source_name || '',
    l: Array.isArray(it.lenses) ? it.lenses : []
  }));
...
async function runDiagnosticCall(cohortContext, items, anthropicKey, batchLabel) {
  ...
  const batchSize = items.length;
  const userMessage = buildDiagnosticUserMessage(cohortContext, items);
  ...
  if (!cleaned) {
    console.error('[SharedResearch] Diagnostic empty content from Anthropic —', 'batch:', batchLabel, 'batch_size:', batchSize, 'input_tokens:', inputTokens, 'output_tokens:', outputTokens);
