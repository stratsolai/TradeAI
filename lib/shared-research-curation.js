...
export async function diagnoseDroppedItems({ cohortContext, droppedItems, anthropicKey }) {
  ...
  const userMessage = buildDiagnosticUserMessage(cohortContext, droppedItems);
  const batchSize = droppedItems.length;
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic fetch exception —', 'batch_size:', batchSize, 'message:', e && e.message);
    return { ok: false, reasons: emptyMap, error: 'fetch exception: ' + (e && e.message), usage: null };
  }

  if (!resp.ok) {
    const errText = await resp.text().catch(() => '');
    console.error('[SharedResearch] Diagnostic non-OK —', 'batch_size:', batchSize, 'status:', resp.status, 'body:', errText.slice(0, 300));
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic HTTP response parse failed —', 'batch_size:', batchSize, 'message:', e && e.message);
    return { ok: false, reasons: emptyMap, error: 'response parse failed: ' + (e && e.message), usage: null };
  }

  if (data && data.error) {
    console.error('[SharedResearch] Diagnostic API error —', 'batch_size:', batchSize, 'error:', JSON.stringify(data.error));
  ...
  if (!cleaned) {
    console.error('[SharedResearch] Diagnostic empty content from Anthropic —', 'batch_size:', batchSize, 'input_tokens:', inputTokens, 'output_tokens:', outputTokens);
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic JSON parse failed —', 'batch_size:', batchSize, 'response_chars:', cleaned.length, 'message:', (e && e.message), 'preview:', cleaned.slice(0, 300));
  ...
  for (const entry of reasonsArr) {
    ...
    if (Number.isInteger(i) && i >= 0 && i < droppedItems.length && reason) {
      reasonsMap.set(i, reason);
    }
  }

  console.log(`[SharedResearch] Diagnostic parse OK — batch_size: ${batchSize}, reasons_returned: ${reasonsMap.size}, input_tokens: ${inputTokens}, output_tokens: ${outputTokens}`);
