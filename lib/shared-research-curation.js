...
//          to move a content item to a different category before
//          dropping it while keeping shared_research at one row per
//          item. See runCuration comment block below for the
//          rationale and the consequence on the items_in / items_out
//          counters.
...
export const CURATION_SYSTEM_PROMPT = [
  ...
  '',
  'RECATEGORISATION DUTY (content items only):',
  'Before dropping a content item (item_type = "content") under Rule 2 (SUBSTANCE TEST) or Rule 3 (RELEVANCE TEST), evaluate whether the item fits any of the OTHER four category definitions above. The item arrives in a batch that reflects which query surfaced it first; that routing is a hint, not a binding constraint on the output category. Re-read the five CATEGORY KEYS above and consider whether the item\'s substance fits a different category. If the item satisfies Rule 2 and Rule 3 under another category\'s definition, retain it and set its output category field to that other category. Drop the item only when no category accepts it under Rule 2 and Rule 3 together. This duty applies to content items only. Listings (item_type = "listing") are not subject to recategorisation — the LISTING CATEGORY AND LENS rule below binds every listing to category = "economic" regardless of any consideration in this duty.',
  '',
...
// (lib/shared-research-plan.js — regulatory → technology → suppliers
...
// dropping a content item under the substance or relevance tests,
// Sonnet must evaluate whether the item fits any of the other four
...
// item. The duty applies to content items only — listings remain
// pinned to category = 'economic' by the Listings Addendum
// invariant, which the validator continues to enforce.
//
...
// an item routed to suppliers but tagged industry-news on output
// counts in suppliers' items_out but lands under industry-news in
// the stored grouping.
//
// Return shape:
...
export async function runCuration({ profile, dedupedItems, anthropicKey }) {
  ...
  // (regulatory → technology → suppliers → economic → industry-news,
