...
//   §8.2 — Finding G dispatcher fix: each item is routed to every
//          category in its source_categories array, not only the
//          first. See runCuration comment block below.
...
// Listings Addendum §3.2 — every curated row carries an item_type.
// 'content' is the default; 'listing' marks marketplace listings
// recognised under the addendum's recognition rule.
export const ITEM_TYPES = ['content', 'listing'];
const REQUIRED_CURATED_FIELDS = ['title', 'summary', 'url', 'source_name', 'category', 'lens', 'item_type'];
...
const VALID_ITEM_TYPE_SET = new Set(ITEM_TYPES);
...
export const CURATION_SYSTEM_PROMPT = [
  ...
  '',
  'ITEM TYPES (every curated output item is tagged with exactly one):',
  '- content — the default. A factual research item: news article, regulatory notice, supplier update, economic data release, technology development. Must pass the substance test in CURATION RULE 2 below.',
  '- listing — a marketplace listing offering a specific business for sale. Subject to the LISTING RECOGNITION rule below, NOT to the substance test. Listings are only retained when they pass the LISTING RELEVANCE GUARDRAILS.',
  '',
  'CURATION RULES:',
  ...
  '',
  'When in doubt at either test, drop rather than retain.',
  ...
  '7. Cap each category at ' + ITEMS_PER_CATEGORY_CAP + ' content items in the output. Choose the most substantive and relevant. Additionally, in the economic category, retain up to 5 listing items (item_type = "listing"). The 5 listings are over and above the ' + ITEMS_PER_CATEGORY_CAP + '-content-item cap, not within it — they are a separate quota. If fewer than 5 genuine listings are available, retain only what\'s available — do not pad. If more than 5 genuine listings are available, choose the 5 most relevant by industry and geography per the LISTING RELEVANCE GUARDRAILS.',
  '8. Australian English throughout (colour, organisation, recognised, etc.).',
  '',
  'LISTING RECOGNITION (exception to rule 2 substance test):',
  'When an input item is a marketplace listing offering a specific business for sale, AND the listing satisfies the LISTING RELEVANCE GUARDRAILS below, retain it and tag it item_type = "listing". Listings that meet the recognition rule are NOT subject to rule 2 — they are a structurally different artefact (an offer to sell, not a report on a fact) and the substance test was written to reject thin journalism, not to reject business-for-sale entries that carry concrete commercial detail.',
  'A genuine listing is an offer to sell a specific business (e.g., "Sydney roofing business", "Newcastle building company for sale", "Gardening business North Shore"). Recognition is a structural test only — does this item read as an offer to sell? Whether the listing carries enough commercial detail to be useful is assessed by the LISTING RELEVANCE GUARDRAILS below, not at this recognition step. Price is one of three quality criteria in guard 3, not a prerequisite for recognition. Generic marketplace pages, category landing pages, hub pages, and aggregator-category listings are NOT listings — they are aggregator navigation and are dropped under rule 2 as content with no substance. The plural form ("Businesses For Sale", "Properties For Sale", etc.) and titles shaped as "[Industry] Businesses For Sale In [Location]" are aggregator-category listings by construction — they aggregate multiple businesses rather than offer a single specific one, and must be dropped regardless of how well the title matches the user\'s industry or region.',
  'When in doubt, prefer item_type = "content". Do NOT relabel a thin content item as a listing to keep it. Listings are a specific format, not a substitute for failed content.',
  '',
  'LISTING RELEVANCE GUARDRAILS (all three must hold):',
  '1. Industry match — the listing names a business in one of the user\'s industries (from the BUSINESS PROFILE block), or a sub-industry that clearly maps to one of them. A listing for an unrelated industry is dropped.',
  '2. Geographic match — the listing names a region, state, or area consistent with the user\'s geographic context. Acceptable in preference order: same region (best), same state, or unspecified-but-national. A listing tied to a different Australian state is dropped.',
  '3. Sufficient detail — the listing provides at least two of: an asking price or price band, a specific business type, a specific location. Pure "business for sale — enquire within" or "established business opportunity" entries with no concrete detail are dropped.',
  '',
  'LISTING CATEGORY AND LENS:',
  '- category for every listing is ALWAYS "economic". Listings represent market-level signals about the local business marketplace and are surfaced from Economic & Market queries. A listing tagged with any other category is invalid and will be rejected.',
  '- lens for a listing follows the same rule as for content: it is the lens array of the source input item(s) that surfaced it. If a listing surfaced from a state-industry query, lens = ["state-industry"]. Dedupe across multiple lenses follows the same combine rule as content.',
  '- source_type for a listing is usually "secondary" (most business-listing aggregators are commercial sites, not government bodies or industry associations). Set "association" only if the listing genuinely sits on an industry association domain.',
  '- summary for a listing is one to two sentences naming what is being sold, the asking price or price band if disclosed, and the location. Stay factual and neutral, the same standard as content summaries.',
  '- published_date may be null when the listing carries no date metadata.',
  '',
  'OUTPUT FORMAT:',
  ...
  '- category: one of the 5 category keys above. Every listing carries category = "economic".',
  '- item_type: exactly "content" or "listing". Default is "content"; use "listing" only when the LISTING RECOGNITION rule applies and all three LISTING RELEVANCE GUARDRAILS are satisfied.',
...
//
// Addendum §8.2 — Finding G dispatcher fix.
//
// Each item is routed to every category in its source_categories
// array, not only the first as the previous implementation did. An
// item surfaced by queries in three categories now goes to all three
// per-category batches; each batch evaluates the item independently
// against its category-specific guidance. The contract: no item is
// routed to fewer categories than appear in its source_categories
// array.
//
// One consequence: items_in / items_out summed across batches no
// longer equals the deduped input size. A single deduped item with
// source_categories = ['regulatory','industry-news'] now counts in
// items_in for both batches. Per-batch totals are about evaluation
// cost; per-stored-category counts are about what landed in
// shared_research after the model picked a final category for each
// kept item. The CLAUDE.md Platform Facts note records this:
// "timings.curation_per_category_breakdown.items_out (per-batch
// returned count) can differ from data.curated_items[category].length
// (per-stored-category count) — two valid measurements at different
// pipeline stages, not a discrepancy."
//
// The model's system prompt still teaches all five categories, so an
// item routed to the regulatory batch can still be tagged technology
// on output based on content — the per-batch routing is about which
// batches get a fair chance at the item, not about pinning its
// output category.
//
// Same-URL/different-category duplicates that can arise when multiple
// batches independently retain an item are deduped after validation
// in the handler (see api/shared-research-refresh.js — dedup by
// url + category preserves the multi-category-per-URL intent while
// preventing literal duplicate rows in shared_research).
//
// Return shape:
...
export async function runCuration({ profile, dedupedItems, anthropicKey }) {
  ...
  // Finding G — route each deduped item to EVERY category in its
  // source_categories array, not only the first. An item that came
  // back from queries in multiple categories now reaches each
  // category's batch and is evaluated against that batch's category
  // guidance. The model is free to re-tag the output to a different
  // category based on content; Finding G ensures every applicable
  // category gets a fair chance at the item rather than the first
  // one winning by query-order accident.
...
function checkValidation(item, normalisedMap) {
  ...
  // Listings Addendum §5 — item_type vocabulary check.
  if (!VALID_ITEM_TYPE_SET.has(item.item_type)) {
    return { reason: `invalid item_type: ${item.item_type}` };
  }

  // Listings Addendum §4.3 + §5 — listings must carry the economic
  // category. A listing tagged with any other category is a curation
  // error and is rejected so the data invariant holds for downstream
  // consumers.
  if (item.item_type === 'listing' && item.category !== 'economic') {
    return { reason: `listing with non-economic category: ${item.category}` };
  }
...
function normaliseCuratedItem(item, normalisedUrl) {
  ...
  return {
    ...
    // Listings Addendum §3 + §5 — item_type is already validated above
    // (REQUIRED + vocabulary check), so we trust the value here.
