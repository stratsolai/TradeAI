...
export const CURATION_SYSTEM_PROMPT = [
  ...
  '',
  'CURATION RULES:',
  '1. Deduplicate items that cover the same story from different URLs before applying any other rule. "Same story" means items reporting on the same underlying event, regulation, organisation, person, or development — judged from the title and snippet. Keep the higher-quality URL, preferring primary sources (government) over association sources, and association sources over secondary sources (general/trade press). Combine the lens arrays of merged items. Near-duplicates count as a single item against the per-category cap (Rule 7).',
  ...
  '  ]',
  '}',
  'Do not emit fields not listed above.',
  '',
  'FIELD RULES:',
  '- title: verbatim from the source. Do not embellish, rewrite, or summarise.',
  '- summary: one to two sentences, factual and specific. Include numbers, dates, agency names where relevant.',
  '- url: one of the URLs from the raw input items, unchanged.',
  ...
  '- published_date: exact value of the date field from the input item, unchanged. If the input item has no date, emit null. Do not infer, calculate, or reformat dates.',
  '',
  'JSON CHARACTER RULES (apply to every string value in your output — title, summary, url, source_type, published_date, and every entry of the lens array):',
  '- The JSON string delimiter is the straight ASCII double quote ("). Every string value opens and closes with ". Any " that appears INSIDE a string value must be escaped as \\". Do not use smart quotes (" " or ‘ ’) anywhere in the output, neither as delimiters nor inside string values. If the source text contains a smart quote or an inner quoted phrase, either rewrite the summary without the inner quotes or escape every inner " as \\" and replace any smart quotes with their straight ASCII equivalents.',
  '- Apostrophes inside string values must be straight ASCII apostrophes (\') and do not need escaping. Do not use smart apostrophes.',
  ...
  '- Before returning, mentally read your output the way a JSON parser would. Each string must open with ", close with ", and contain only characters valid inside a JSON string. Trailing commas are invalid.',
  '',
...
export function buildCurationUserMessage(cohortContext, dedupedItems) {
  ...
  // SRL SME-Lens Scope Separation v1.1 §9 — when industries is empty,
  // this is an SME-scope curation pass. The system prompt's Rule 3b
  // industry-specific clause cannot fire (no industries to match), so
  // we add one explicit line to remove any ambiguity for the model:
  // apply the cross-cutting clause, ignore the industry-specific path.
  // The system prompt itself stays unchanged; the SME case is signalled
  // entirely via the user message.
  const smeScopeNote = industries.length === 0
    ? 'NOTE: This is an SME-scope curation pass — there is no specific industry context. Apply Rule 3b\'s cross-cutting clause; the industry-specific clause does not apply.'
    : null;

  const profileBlockLines = [
    'BUSINESS PROFILE',
    'Industries: ' + (industries.length ? industries.join(' | ') : 'unspecified'),
    'State: ' + (state || 'unspecified'),
    'Region: ' + (region || 'unspecified')
  ];
  if (smeScopeNote) profileBlockLines.push('', smeScopeNote);
  const profileBlock = profileBlockLines.join('\n');
...
function normaliseCuratedItem(item, normalisedUrl) {
  ...
  // source_name and source_domain are NOT read from Sonnet's emission.
  // The refresh handler attribution stage overrides source_name with
  // the Serper source-of-truth value and computes source_domain from
  // the URL — both fields end up on the persisted row via those
  // overrides, not via Sonnet's output. The string-default below keeps
  // source_name in the row shape stable; source_domain is added by the
  // handler's overrides assignment.
  return {
    item: {
...
// ---------------------------------------------------------------------------
// Diagnostic — reconstruct why each not_returned_by_sonnet item was dropped
// ---------------------------------------------------------------------------
//
// Observation-only helper for the Task 46 calibration sweep. Runs ONLY on
// the dry-run admin path AFTER runCuration has completed and the
// curation keep/drop set is final. The diagnostic is structurally unable
// to influence curation — by the time it runs, runCuration has already
// returned and the response is being assembled.
//
// What it does: takes the items the production curation pass dropped
// (i.e. items that were submitted to Sonnet but were not returned by it)
// and asks a second Sonnet call to write a short reason for each one,
// referenced against the same Substance / Geography / Topic tests
// the curation prompt applies.
//
// What it does NOT do: it does not change the production curation
// system prompt, does not re-evaluate keep/drop, and is not part of
// the chain that produced the keep/drop set. The reasons are a best-
// effort reconstruction of the most likely rejection cause — the
// original Sonnet call's internal reasoning is not surfaced anywhere
// in its response, so this is a separate prompt designed to mirror
// the same rule vocabulary.
//
// Returns: { ok, reasons: Map<index, string>, error, usage } — reasons is
// keyed by the index into droppedItems passed in, so the caller can
// attach each reason to its specific not_returned_by_sonnet entry.

const DIAGNOSTIC_MAX_TOKENS = 4000;
const DIAGNOSTIC_INPUT_TRUNCATE = 600;

// Maximum dropped items per single Sonnet call. At ~30-40 output tokens
// per item, 50 items fits comfortably under DIAGNOSTIC_MAX_TOKENS (~2,000
// tokens used vs 4,000 ceiling — 2x headroom for variability in reason
// length). diagnoseDroppedItems chunks the input at this size and fires
// all chunks in parallel; the merged reason map covers every item that
// any batch returned a reason for.
const DIAGNOSTIC_BATCH_SIZE = 50;

const DIAGNOSTIC_SYSTEM_PROMPT = [
  'You are a curation diagnostics assistant for StaxAI, an Australian SME research platform.',
  '',
  'Each item you receive in the user message has ALREADY been dropped by an earlier curation pass. The keep/drop decision is final. Your job is NOT to re-decide; your job is to write a short reason explaining why each item would not be considered relevant for an Australian SME business audience.',
  '',
  'The earlier curation pass applies these tests, drawn from the production curation prompt:',
  '',
  '- SUBSTANCE TEST — items that do not name a specific thing (regulation, policy, event, organisation, person, named development) fail. Vague clickbait titles ("what you need to know", "trends to watch", "top X tips", "your essential guide to", "X things to consider"), listicles, and SEO-driven articles fail by construction.',
  '- GEOGRAPHY TEST — items whose primary subject is a foreign country, foreign region, or foreign city fail. Items carrying a state lens must be primarily about that state, multiple Australian states, or Australia as a whole. Items carrying a region lens must be primarily about that region, its state, or Australia as a whole.',
  '- TOPIC TEST — items must reasonably affect an Australian SME owner\'s commercial, operational, regulatory, or financial interests. Consumer DIY, lifestyle, travel, entertainment, hobbyist, individual-personal-circumstance, civic or constitutional content not specific to business, and tourism fail. Vendor product launches and content marketing fail when their value depends on the named vendor or product.',
  '',
  'For each item, pick the single most likely reason and write one short sentence (under 30 words). Reference the failing test by name where it helps ("Fails Geography Test — primarily about the UK"). Do not editorialise, do not recommend, do not write more than one sentence per item.',
  '',
  'OUTPUT FORMAT:',
  'Return ONLY a JSON object in this exact shape, no preamble, no markdown fences:',
  '{',
  '  "reasons": [',
  '    { "i": 0, "reason": "Fails Substance Test — title is a vague listicle with no named regulation." },',
  '    { "i": 1, "reason": "Fails Geography Test — primarily about a UK retailer." }',
  '  ]',
  '}',
  '',
  'Emit one entry per input item, keyed by the i value from the input. Do not invent additional indices. Each reason must be a single short sentence, plain ASCII, no smart quotes, no newlines.',
  '',
  'Return ONLY the JSON object.'
].join('\n');

function buildDiagnosticUserMessage(cohortContext, droppedItems) {
  const ctx = cohortContext || {};
  const industries = Array.isArray(ctx.industries) ? ctx.industries : [];
  const state = typeof ctx.state === 'string' && ctx.state ? ctx.state : null;
  const region = typeof ctx.region === 'string' && ctx.region ? ctx.region : null;

  // Defensive coercion — every field is forced through String() before any
  // string method (.slice, .trim) is applied. The deduped item pipeline is
  // supposed to deliver strings on title/snippet/body/link/source, but a
  // single unexpected non-string value (null in a place we don't anticipate,
  // an object on .body from a future scraper change, etc.) would otherwise
  // throw "x.slice is not a function" out of an unprotected synchronous
  // line and 500 the entire dry-run. Coercing here means worst-case the
  // model sees a stringified "[object Object]" — not great, but the
  // diagnostic still completes and the rest of the response delivers.
  const compact = droppedItems.map((it, i) => {
    const safe = it || {};
    const titleStr = safe.title == null ? '' : String(safe.title);
    const bodyStr = safe.body == null ? '' : String(safe.body);
    const snippetStr = safe.snippet == null ? '' : String(safe.snippet);
    const linkStr = safe.link == null ? (safe.url == null ? '' : String(safe.url)) : String(safe.link);
    const srcStr = safe.source == null ? (safe.source_name == null ? '' : String(safe.source_name)) : String(safe.source);
    const content = (bodyStr || snippetStr || '').slice(0, DIAGNOSTIC_INPUT_TRUNCATE);
    return {
      i,
      t: titleStr,
      s: content,
      u: linkStr,
      src: srcStr,
      l: Array.isArray(safe.lenses) ? safe.lenses : []
    };
  });

  const profileBlock = [
    'BUSINESS PROFILE (for reference only — these items have already been dropped)',
    'Industries: ' + (industries.length ? industries.join(' | ') : 'unspecified'),
    'State: ' + (state || 'unspecified'),
    'Region: ' + (region || 'unspecified')
  ].join('\n');
  ...
  const itemsBlock = [
    'DROPPED ITEMS (compact format — keys: i=index, t=title, s=snippet, u=url, src=source, l=lenses)',
    JSON.stringify(compact)
  ].join('\n');
  ...
  return [
    ...
    itemsBlock,
    '',
    'Write one short reason per item. Return only the JSON object.'
  ].join('\n');
}

// Single-batch diagnostic call. Items are local to the batch — the returned
// reasons map is keyed by local index (0..items.length-1). The public
// diagnoseDroppedItems wrapper below translates local indices to global
// indices using each batch's start offset, then merges into one map.
//
// batchLabel is a human-readable identifier surfaced in every log line so
// per-batch outcomes can be correlated across the parallel fan-out
// (e.g. "0-49" or "50-99" — a slice descriptor matching the global
// range of items this batch covers).
async function runDiagnosticCall(cohortContext, items, anthropicKey, batchLabel) {
  const emptyMap = new Map();
  const batchSize = items.length;

  // Early log so a future failure that throws before reaching the per-batch
  // success/error logs still leaves a breadcrumb identifying which batch
  // started. Tracks down "Dispatching" fired but no per-batch log followed
  // — the failure mode that caused the 500 we hardened against here.
  console.log(`[SharedResearch] Diagnostic batch start — batch: ${batchLabel}, batch_size: ${batchSize}`);

  // Wrap the user-message build. JSON.stringify of the compact array is
  // the one synchronous step before fetch that could plausibly throw on
  // an unexpected input shape; coercion above defends against it, this
  // catch is the second line of defence.
  let userMessage;
  try {
    userMessage = buildDiagnosticUserMessage(cohortContext, items);
  } catch (e) {
    console.error('[SharedResearch] Diagnostic user message build failed —', 'batch:', batchLabel, 'batch_size:', batchSize, 'message:', e && e.message, 'stack:', e && e.stack);
    return { ok: false, reasons: emptyMap, error: 'user message build failed: ' + (e && e.message), usage: null };
  }
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic fetch exception —', 'batch:', batchLabel, 'batch_size:', batchSize, 'message:', e && e.message);
    return { ok: false, reasons: emptyMap, error: 'fetch exception: ' + (e && e.message), usage: null };
  }

  if (!resp.ok) {
    const errText = await resp.text().catch(() => '');
    console.error('[SharedResearch] Diagnostic non-OK —', 'batch:', batchLabel, 'batch_size:', batchSize, 'status:', resp.status, 'body:', errText.slice(0, 300));
    return { ok: false, reasons: emptyMap, error: 'non-OK status: ' + resp.status, usage: null };
  }
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic HTTP response parse failed —', 'batch:', batchLabel, 'batch_size:', batchSize, 'message:', e && e.message);
    return { ok: false, reasons: emptyMap, error: 'response parse failed: ' + (e && e.message), usage: null };
  }

  if (data && data.error) {
    console.error('[SharedResearch] Diagnostic API error —', 'batch:', batchLabel, 'batch_size:', batchSize, 'error:', JSON.stringify(data.error));
    return { ok: false, reasons: emptyMap, error: 'api error: ' + (data.error.message || 'unknown'), usage: data.usage || null };
  }

  // Explicit string check before .replace — guards against the rare case
  // where data.content[0] is a non-text block (tool_use, etc.) or any
  // other shape where .text isn't a string. Without this, .replace would
  // throw synchronously and the async function would reject.
  const rawCandidate = data && data.content && data.content[0] && data.content[0].text;
  const raw = typeof rawCandidate === 'string' ? rawCandidate : '';
  const cleaned = raw.replace(/```json|```/g, '').trim();
  const inputTokens = (data && data.usage && data.usage.input_tokens) || 0;
  const outputTokens = (data && data.usage && data.usage.output_tokens) || 0;

  if (!cleaned) {
    const rawType = typeof rawCandidate;
    console.error('[SharedResearch] Diagnostic empty content from Anthropic —', 'batch:', batchLabel, 'batch_size:', batchSize, 'input_tokens:', inputTokens, 'output_tokens:', outputTokens, 'raw_type:', rawType);
    return { ok: false, reasons: emptyMap, error: 'empty content from Anthropic', usage: data.usage || null };
  }
  ...
  try {
    ...
    console.error('[SharedResearch] Diagnostic JSON parse failed —', 'batch:', batchLabel, 'batch_size:', batchSize, 'response_chars:', cleaned.length, 'message:', (e && e.message), 'preview:', cleaned.slice(0, 300));
    return { ok: false, reasons: emptyMap, error: 'json parse failed: ' + (e && e.message), usage: data.usage || null };
  }

  const reasonsArr = (parsed && Array.isArray(parsed.reasons)) ? parsed.reasons : [];
  const reasonsMap = new Map();
  for (const entry of reasonsArr) {
    if (!entry || typeof entry !== 'object') continue;
    const i = entry.i;
    const reason = typeof entry.reason === 'string' ? entry.reason.trim() : '';
    if (Number.isInteger(i) && i >= 0 && i < batchSize && reason) {
      reasonsMap.set(i, reason);
    }
  }

  console.log(`[SharedResearch] Diagnostic parse OK — batch: ${batchLabel}, batch_size: ${batchSize}, reasons_returned: ${reasonsMap.size}, input_tokens: ${inputTokens}, output_tokens: ${outputTokens}`);

  return { ok: true, reasons: reasonsMap, error: null, usage: data.usage || null };
}

// Public diagnostic entry point. Chunks dropped items into batches of
// DIAGNOSTIC_BATCH_SIZE, fires each batch in parallel against Sonnet,
// then merges the per-batch reason maps into a single map keyed by the
// caller's global index.
//
// Partial-failure handling: each batch is independent. If batch 2 errors
// (network, parse failure, API error, or truncation) while batches 1
// and 3 succeed, the merged map carries reasons for items in batches 1
// and 3, and items in batch 2 are simply absent from the map (the caller
// renders absent reasons as null per the existing contract). ok is true
// only when every batch succeeded; error carries the first batch
// failure surfaced when any batch failed. usage sums input/output
// tokens across every batch that returned usage, so cost attribution
// reflects the full per-refresh diagnostic spend.
//
// The reasons Map is callable-compatible with the single-call contract
// it replaces: same Map<globalIndex, string> shape, same emptiness
// semantics on input edge cases.

export async function diagnoseDroppedItems({ cohortContext, droppedItems, anthropicKey }) {
  const emptyMap = new Map();

  if (!anthropicKey) {
    return { ok: false, reasons: emptyMap, error: 'ANTHROPIC_API_KEY missing', usage: null };
  }
  if (!Array.isArray(droppedItems) || droppedItems.length === 0) {
    return { ok: true, reasons: emptyMap, error: null, usage: null };
  }

  // Top-level safety net. runDiagnosticCall is structured to resolve in
  // every failure mode (try/catches around fetch, json parse, etc.), but
  // an unanticipated throw — Promise.all rejecting on a malformed inner
  // promise, an uncaught sync throw in the merge loop, a future code
  // change that breaks the resolve-never-reject contract — should not
  // turn into a 500 on the dry-run handler. The diagnostic is
  // observation-only; a failure to produce reasons must never block the
  // dry-run from returning the rest of the response.
  //
  // On unhandled throw we log the error with stack and return the same
  // shape any other failure path uses, so the caller's logic doesn't
  // need a special case.
  try {
    // Build chunks. Each chunk carries its global start offset so reasons
    // can be re-indexed back to the caller's frame after the call returns.
    const chunks = [];
    for (let start = 0; start < droppedItems.length; start += DIAGNOSTIC_BATCH_SIZE) {
      const end = Math.min(start + DIAGNOSTIC_BATCH_SIZE, droppedItems.length);
      chunks.push({
        start,
        label: `${start}-${end - 1}`,
        items: droppedItems.slice(start, end)
      });
    }

    console.log(`[SharedResearch] Diagnostic dispatching — total_items: ${droppedItems.length}, batch_size: ${DIAGNOSTIC_BATCH_SIZE}, batch_count: ${chunks.length}`);

    // Fire all batches in parallel. Promise.all preserves order so chunks[i]
    // and results[i] correspond. Each runDiagnosticCall resolves (never
    // rejects) by design — internal errors are returned as { ok: false }.
    const results = await Promise.all(chunks.map((c) =>
      runDiagnosticCall(cohortContext, c.items, anthropicKey, c.label)
    ));
    ...
    for (let bi = 0; bi < chunks.length; bi++) {
      const r = results[bi];
      const start = chunks[bi].start;
      const label = chunks[bi].label;

      // Defensive — every runDiagnosticCall return must have the standard
      // four-field shape. If a future refactor breaks the contract the
      // merge stays safe rather than throwing on an undefined .ok.
      if (!r || typeof r !== 'object') {
        if (!firstError) firstError = `batch ${label}: malformed result`;
        continue;
      }

      if (r.ok) {
        okBatchCount++;
        if (r.reasons instanceof Map) {
          for (const [localI, reason] of r.reasons) {
            mergedReasons.set(start + localI, reason);
          }
        }
      } else if (!firstError) {
        firstError = `batch ${label}: ${r.error}`;
      }
      if (r.usage) {
        anyUsageSeen = true;
        aggregateInputTokens += r.usage.input_tokens || 0;
        aggregateOutputTokens += r.usage.output_tokens || 0;
      }
    }

    const allOk = okBatchCount === chunks.length;
    console.log(`[SharedResearch] Diagnostic merge complete — batches_ok: ${okBatchCount}/${chunks.length}, reasons_merged: ${mergedReasons.size}/${droppedItems.length}, input_tokens: ${aggregateInputTokens}, output_tokens: ${aggregateOutputTokens}`);
    ...
    console.error('[SharedResearch] Diagnostic unhandled exception —', 'total_items:', droppedItems.length, 'message:', (e && e.message), 'stack:', (e && e.stack));
    return {
      ok: false,
      reasons: emptyMap,
      error: 'unhandled exception: ' + ((e && e.message) || 'unknown'),
      usage: null
    };
  }
}
