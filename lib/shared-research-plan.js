...
// SRL SME-Lens Scope Separation v1.1 §4 — the lens filter applied to
// buildQueryPlan based on the requested scope. Cohort refreshes fire
// only the three industry-bearing lenses (national/state/region-
// industry); each SME scope tier fires exactly the matching SME lens.
// The previously-monolithic "everything in one plan" behaviour is no
// longer produced by this function — callers always pass a scope.
const LENSES_BY_SCOPE = {
  cohort:   new Set(['national-industry', 'state-industry', 'region-industry']),
  national: new Set(['national-smes']),
  state:    new Set(['state-smes']),
  region:   new Set(['region-smes'])
};

export function getLensesForScope(scope) {
  return LENSES_BY_SCOPE[scope] || null;
}

export function buildQueryPlan(profile, options) {
  options = options || {};
  // Optional displayLabel-keyed override map for substitution phrases —
  // only used by the dry-run calibration path in
  // api/shared-research-refresh.js. Caller is responsible for shape
  // validation (non-empty array of non-empty trimmed strings per
  // industry); we treat the map as authoritative here.
  const substitutionOverrides = options.substitutionOverrides || null;
  // SRL SME-Lens Scope Separation v1.1 §4 — scope defaults to 'cohort'
  // (the post-split cohort path which fires only industry-lens queries).
  // SME scopes fire only the matching SME lens. Unknown scopes throw so
  // a typo surfaces immediately rather than producing an empty plan.
  const scope = options.scope || 'cohort';
  const allowedLenses = LENSES_BY_SCOPE[scope];
  if (!allowedLenses) {
    throw new Error('[shared-research-plan] Unknown scope: ' + scope);
  }
  ...
  for (const categoryKey of Object.keys(CATEGORIES)) {
    ...
    for (const lens of LENSES) {
      // Skip lenses outside the requested scope. Belt-and-braces with
      // the per-lens skips below — if a scope requests a lens that
      // depends on state/region but the profile lacks them, the per-
      // lens skips still apply.
      if (!allowedLenses.has(lens)) continue;
      ...
      if (INDUSTRY_AGNOSTIC_LENSES.has(lens)) {
        ...
        for (const industry of industries) {
          ...
          let substitutions;
          if (substitutionOverrides && Array.isArray(substitutionOverrides[industry]) && substitutionOverrides[industry].length > 0) {
            // Caller-supplied override wins. Validation happened in the
            // handler; trust the array here.
            substitutions = substitutionOverrides[industry];
          } else if (taxEntry && Array.isArray(taxEntry.srlSubstitution) && taxEntry.srlSubstitution.length > 0) {
