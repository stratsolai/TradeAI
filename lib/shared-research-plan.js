...
// ---------------------------------------------------------------------------
// Listings templates — Phase 6 Pass 4 (inline supersession of Listings
// Addendum §4.1's "listings have no dedicated queries" statement)
// ---------------------------------------------------------------------------
//
// Three additional templates that fire alongside the existing economic
// news queries on the three industry-specific lens cells. Each cell
// emits BOTH its existing news query AND a listings query per industry.
//
// query_type is 'search' (Web search, not News) — listings sit on
// aggregator domains (BusinessesForSale.com.au, SeekBusiness.com.au,
// etc.) that are not indexed by Google News. The News endpoint
// returns ~0 useful results for these queries; Web search is where
// the listings live.
//
// Recency is qdr:m3 (3 months) regardless of the economic category's
// own qdr:m setting — listings linger on aggregator sites longer than
// news stays fresh, and a 1-month window cuts genuine in-market entries.
//
// Style follows the parent spec §7.4 — short natural language, no site:
// operators, no quoted phrases, no location operators. The [industry]
// token is the user's industry display name exactly as stored in the
// Business Profile (ampersands preserved).
const LISTINGS_TEMPLATES = {
  'national-industry': ({ industry })            => `businesses related to ${industry} for sale Australia`,
  'state-industry':    ({ industry, stateFull }) => `businesses related to ${industry} for sale ${stateFull}`,
  'region-industry':   ({ industry, region })    => `businesses related to ${industry} for sale ${region}`
};
const LISTINGS_RECENCY = 'qdr:m3';
const LISTINGS_QUERY_TYPE = 'search';
    ...
    for (const lens of LENSES) {
      ...
      if (INDUSTRY_AGNOSTIC_LENSES.has(lens)) {
        ...
        for (const industry of industries) {
          ...
          // Listings emit — Phase 6 Pass 4. Only the three economic
          // industry-specific cells fire listings queries; everything
          // else is news-only. The listings query runs alongside the
          // news query, per industry, with its own query_type and
          // recency.
          if (categoryKey === 'economic' && LISTINGS_TEMPLATES[lens]) {
            const listingQuery = LISTINGS_TEMPLATES[lens]({
              industry,
              stateFull,
              region: region ? region.region_name : null
            });
            if (listingQuery) {
              plan.push({
                ...
                industry,
                query: listingQuery,
                query_type: LISTINGS_QUERY_TYPE,
                recency: LISTINGS_RECENCY,
                intent: 'listings',
                location
              });
            }
