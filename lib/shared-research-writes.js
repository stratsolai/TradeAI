...
export function buildSharedResearchRow(userId, refreshId, curatedItem) {
  ...
  // (per addendum §7), but the addendum also requires the distinction
  // be intentional and auditable, not inherited from a default. The
  // curation validator (REQUIRED_CURATED_FIELDS + VALID_ITEM_TYPE_SET)
  // guarantees curatedItem.item_type is set to 'content' or 'listing'
  // by the time we get here; the || 'content' fallback is belt-and-
  // braces for any direct caller that somehow bypasses validation.
  return {
    user_id: userId,
...
//
// access_type vocabulary now has four values:
//   'read_hit'              — Phase 3, cache read returned a row
//   'read_miss'             — Phase 3, cache read returned nothing
//   'write'                 — Phase 3, cache row written after fetch
//   'shared_research_write' — Phase 4, curated batch written to
//                             shared_research after a successful refresh
//
