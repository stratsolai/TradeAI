...
// SRL SME-Lens Scope Separation v1.1 §5.1 — every read/update in this
// file is now scoped by (scope_type, scope_key, cohort_id). Cohort
// scope: scope_type='cohort', cohort_id set, scope_key null. National:
// scope_type='national', cohort_id null, scope_key null. State:
// scope_type='state', cohort_id null, scope_key='<state-lc>'. Region:
// scope_type='region', cohort_id null, scope_key='<state-lc>::<region-
// slug>'. The flip and rollback must never cross scopes — an SME
// refresh in (state, NSW) cannot touch cohort rows or other states.
function applyScopeFilter(query, scope) {
  query = query.eq('scope_type', scope.scope_type);
  if (scope.scope_type === 'cohort') {
    query = query.eq('cohort_id', scope.cohort_id);
  } else if (scope.scope_type === 'state' || scope.scope_type === 'region') {
    query = query.eq('scope_key', scope.scope_key);
  }
  // 'national' scope: scope_type alone identifies it; no scope_key
  // filter (the column is null for national rows by design).
  return query;
}

function scopeLogTag(scope) {
  if (scope.scope_type === 'cohort') return 'cohort_id: ' + scope.cohort_id;
  if (scope.scope_type === 'national') return 'scope: national';
  return 'scope: ' + scope.scope_type + '/' + scope.scope_key;
}

export async function snapshotCurrentRowIds(supabase, scope) {
  try {
    let q = supabase.from('shared_research').select('id').eq('is_current', true);
    q = applyScopeFilter(q, scope);
    const res = await q;
    if (res.error) {
      console.error('[SharedResearch] is_current snapshot error —', scopeLogTag(scope), 'message:', res.error.message);
      return { ok: false, ids: [], error: res.error.message };
    }
    return { ok: true, ids: (res.data || []).map((r) => r.id), error: null };
  } catch (e) {
    console.error('[SharedResearch] is_current snapshot exception —', scopeLogTag(scope), 'message:', e && e.message);
    return { ok: false, ids: [], error: e && e.message };
  }
}

export async function flipIsCurrentFalse(supabase, scope) {
  try {
    let q = supabase.from('shared_research').update({ is_current: false }).eq('is_current', true);
    q = applyScopeFilter(q, scope);
    const res = await q;
    if (res.error) {
      console.error('[SharedResearch] is_current flip error —', scopeLogTag(scope), 'message:', res.error.message);
      return { ok: false, error: res.error.message };
    }
    return { ok: true, error: null };
  } catch (e) {
    console.error('[SharedResearch] is_current flip exception —', scopeLogTag(scope), 'message:', e && e.message);
    return { ok: false, error: e && e.message };
  }
}

export async function restoreIsCurrent(supabase, scope, ids) {
  if (!Array.isArray(ids) || ids.length === 0) return { ok: true, error: null };
  try {
    let q = supabase.from('shared_research').update({ is_current: true }).in('id', ids);
    q = applyScopeFilter(q, scope);
    const res = await q;
    if (res.error) {
      console.error('[SharedResearch] is_current rollback error —', scopeLogTag(scope), 'count:', ids.length, 'message:', res.error.message);
      return { ok: false, error: res.error.message };
    }
    return { ok: true, error: null };
  } catch (e) {
    console.error('[SharedResearch] is_current rollback exception —', scopeLogTag(scope), 'count:', ids.length, 'message:', e && e.message);
    return { ok: false, error: e && e.message };
  }
}
...
// SRL SME-Lens Scope Separation v1.1 §5 — every row carries scope_type
// and, for state/region SME rows, scope_key. Cohort rows continue to
// carry cohort_id and leave scope_key null; SME rows have cohort_id
// null and scope_key holds the geography.
export function buildSharedResearchRow(scopeAndRefresh, curatedItem) {
  return {
    cohort_id: scopeAndRefresh.cohort_id || null,
    scope_type: scopeAndRefresh.scope_type,
    scope_key: scopeAndRefresh.scope_key || null,
    refresh_id: scopeAndRefresh.refresh_id,
...
// ---------------------------------------------------------------------------
// Refresh row error update — used on rollback paths
// ---------------------------------------------------------------------------
//
// Under the parent-first write ordering (Phase 7 fix), the refresh row
// is INSERTed before the items it parents. If the subsequent item
// writes fail (flip or insert), the refresh row already exists with
// an optimistic outcome and stats; this function updates it to
// reflect reality:
//   - outcome flips to 'error'
//   - stats is rewritten with the supplied object (typically the same
//     stats shape but with curated_items: 0 since none landed)
//   - completed_at is bumped to "now" — the failure moment is when
//     the pipeline actually finished, not when the optimistic row
//     was first written
//
// Stats is rewritten as a whole jsonb object rather than patched via
// jsonb_set because Supabase JS does not expose jsonb_set cleanly and
// the caller already has the full stats shape in hand.

export async function updateRefreshRowToError(supabase, refreshId, stats) {
  try {
    const res = await supabase
      ...
      .eq('id', refreshId);
    if (res.error) {
      console.error('[SharedResearch] Refresh row error-update failed —', 'refresh_id:', refreshId, 'message:', res.error.message);
      return { ok: false, error: res.error.message };
    }
    return { ok: true, error: null };
  } catch (e) {
    console.error('[SharedResearch] Refresh row error-update exception —', 'refresh_id:', refreshId, 'message:', e && e.message);
    return { ok: false, error: e && e.message };
  }
}
...
export async function writeRefreshRow(supabase, row) {
  try {
    // completed_at is the moment of insert by definition — this function
    // owns it so the caller can't forget. created_at is the Postgres
    // default (DB-server clock at INSERT); completed_at is the
    // application-level "refresh pipeline finished" timestamp. The two
    // are nearly identical in practice but semantically distinct, so
    // both are populated.
    //
    // If a caller explicitly passes completed_at (e.g. a future
    // backfill tool), that wins over the default below.
    const rowOut = Object.assign(
      { completed_at: new Date().toISOString() },
      row
    );
    const res = await supabase
