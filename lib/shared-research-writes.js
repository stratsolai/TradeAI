...
//
// Initial Phase 4 implementation introduced a new access_type value
// 'shared_research_write'. Owner testing showed those rows weren't
// being persisted — the access_type column is constrained (CHECK or
// enum) to the three existing Phase 3 values, so the inserts were
// rejected by Postgres and only surfaced in Vercel logs. Adding the
// new value to the constraint would require an owner-side SQL change
// per CLAUDE.md ("All SQL provided by Chat"). The Phase 4 brief
// explicitly allows the alternative path: reuse the existing 'write'
// access_type with a distinguishing tag.
//
// Convention adopted:
//   access_type = 'write'           (an existing accepted value)
//   cache_key   = 'shared_research:<refreshId>'
//
// Real cache writes have cache_key set to a 64-char SHA-256 hex
// digest from buildCacheKey() — the 'shared_research:' prefix is
// unambiguously distinguishable. Verification queries filter by
// cache_key LIKE 'shared_research:%' (see the verification SQL in
// the Phase 4 sign-off report).
//
