...
// Records one row per cache interaction in shared_research_cache_access
// so per-user audit (which user read which cached row, which user
// triggered a fresh fetch) is preserved. Failures here must never
// break the response — the data has already been served by the time
// we're logging. access_type vocabulary matches the schema:
//   - 'read_hit'  : user found and used an existing cache row
//   - 'write'     : user triggered a fresh fetch and the row was written
//   ('read_miss' is reserved in the schema but not currently emitted —
//    the user-facing event is the subsequent 'write'.)

async function logCacheAccess(supabase, cacheKey, userId, accessType) {
  if (!userId || !cacheKey) return;
  try {
    const res = await supabase
      .from('shared_research_cache_access')
      .insert({ cache_key: cacheKey, user_id: userId, access_type: accessType });
    if (res.error) console.error('[SharedResearch] Cache access log error —', 'type:', accessType, 'message:', res.error.message);
  } catch (e) {
    console.error('[SharedResearch] Cache access log exception —', 'type:', accessType, 'message:', e && e.message);
  }
}
...
export async function runSerperQuery({ query, queryType, tbs, apiKey }) {
  ...
  try {
    ...
    // exact response Serper sent, byte for byte. Functionally equivalent
    // to the previous resp.json() — only the timing of the parse moves.
    let rawText = '';
...
export async function executeQueryWithCache({ supabase, userId, planRow, apiKey, forceRefresh }) {
  ...
  if (!forceRefresh) {
    ...
    if (hit) {
      ...
      await logCacheAccess(supabase, cacheKey, userId, 'read_hit');
      return {
    ...
    // access table before we move on to fetch. Only fires on a genuine
    // miss, not on force_refresh (force_refresh bypasses the read, so
    // there is no read event to log).
    await logCacheAccess(supabase, cacheKey, userId, 'read_miss');
  }
  ...
  if (fresh.ok) {
    ...
    await logCacheAccess(supabase, cacheKey, userId, 'write');
  }
  return {
    items: fresh.items,
    cache_hit: false,
