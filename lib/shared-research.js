...
export function buildCurationUserMessage(profile, dedupedItems) {
  ...
  const itemsBlock = [
    'RAW WEB RESULTS (compact format — keys: i=index, t=title, s=snippet, u=url, src=source, d=date, c=category, l=lenses)',
...
function normaliseCuratedItem(item) {
  const lenses = Array.isArray(item.lens) ? item.lens : [item.lens];
  const sourceType = VALID_SOURCE_TYPE_SET.has(item.source_type) ? item.source_type : 'secondary';
  return {
