...
// duration_ms, ok, error, and items_returned for handler-level
...
export async function runCuration({ profile, dedupedItems, anthropicKey }) {
  ...
  // { category, ok, items, error, usage, duration_ms }.
  const batchPromises = CURATED_CATEGORIES.map(async (category) => {
    ...
    return Object.assign({ category }, result, { duration_ms: Date.now() - t0 });
  });
